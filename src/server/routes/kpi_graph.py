...
async def get_kpi_graph_summary(
    ...
    workbook_ids: Optional[str] = Query(None),
    view: str = Query("landscape", description="landscape | rationalization"),
    ...
    if view == "rationalization":
        nodes, links, matched = build_rationalization_graph(workbook_ids=wb_ids)
        matched = [{"name": w["name"], "workbook_name": w["name"], "line_of_business": None, "ai_summary": w.get("purpose")} for w in matched]
    else:
        nodes, links, matched = build_graph_data(
            cleaned_dashboards,
            workbook_id=workbook_id,
            workbook_ids=wb_ids,
        )
    ...
    nodes_str = "\n".join([f"- {n['group']}: {n['label']}" for n in nodes[:50]])
    links_str = "\n".join([f"- {l['source']} --({l['label']})--> {l['target']}" for l in links[:60]])
    dash_summaries = "\n".join([
        f"Dashboard Name: {d['name']}\nWorkbook: {d['workbook_name']}\n"
        f"LOB: {d.get('line_of_business')}\nAI Summary: {d.get('ai_summary') or 'N/A'}"
        for d in matched
    ])

    prompt = f"""You are a BI governance expert analyzing a rationalization graph of Excel workbooks.

Focus: which workbooks overlap on KPIs, datasources, and formulas — and which should be kept, merged, or decommissioned.
The user is highlighting the '{focus_type}' view in the graph.
...
- PROVIDE EXACTLY 3 to 4 bullet points. Focus on overlap and rationalization decisions.
...
- Highlight duplicate workbooks, merge candidates, and shared KPIs across files.
